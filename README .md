...
## Introduction
This project is intended to build and deploy a person detection on CM2290 with SNPE on android application using YOLOV5 model with Integration on Thundercomm TurboX CM2290 LA (Linux Android) DK (Development Kit), which helps to deploy AI Camera Applications on edge device. An android person detection API identify person's from live video, using Deep Learning model. 
...
- Connect the Wi-Fi
- Install vyser for screen mirroring .  Refer link https://www.vysor.io/download/linux 

## 1. Steps to create android application 
### 1.1 Clone the project
```sh
     $ git clone <source repository>
     $ cd Person-Detection-With-YOLOV5-on-CM2290-with-SNPE/
```
...
3. In the NDK section select version 17.2 version and CMake of 3.18.1 and click on apply. This will download the Android NDK and CMake required by SNPE SDK.

4. Go to ‘CMakeLists.txt’ file and make the following changes for setting up OPENCV_DIR and include directories of SNPE SDK.
...
## 2. Steps to Run Person-Detection-With-YOLOV5-on-CM2290-with-SNPE Application:
...
```sh
  ...
  - $ adb shell
    $ adb push Person-Detection-With-YOLOV5-on-CM2290-with-SNPE/models/yolov5_person.dlc /storage/emulated/0/appData/models/  
   
```   

## 3. Steps for running the main application are as follows:

1. Go to android studio and generate APK file for Person-Detection-With-YOLOV5-on-CM2290-with-SNPE.
    ...
    Person-Detection-With-YOLOV5-on-CM2290-with-SNPE/app/build/outputs/apk/debug/app-debug.apk
...
4. First give permission to the application for accessing camera, accessing storage and record audio. It will open the live camera and keeps taking the camera frames.
<img src="outputs/permissions.png" alt="Alt text" title="Optional title">
<img src="outputs/permissions_1.png" alt="Alt text" title="Optional title">

5. Detected image will be saved on the device in model folder.

6. Download image/video from device to host system.
```sh
   $ adb pull /storage/emulated/0/models/output.jpg /home/
   $ adb pull /storage/emulated/0/models/output_video.avi /home/
```
