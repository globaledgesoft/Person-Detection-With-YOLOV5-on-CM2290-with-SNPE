...
Java_com_example_persondetection_MainActivity_onCreateJNI(JNIEnv *env, jobject clazz, jobject activity, jobject j_asset_manager) {
    LOGI("+++++++++++++++++++++++++++++++++++++++");

    std::stringstream path;
    //path << "/system/lib" << "/data/local/tmp/snpeexample/dsp/lib;/system/lib/rfsa/adsp;/system/vendor/lib/rfsa/adsp;/dsp;/vendor/lib";
    path << "/data/local/tmp/snpeexample/dsp/lib;/system/lib/rfsa/adsp;/system/vendor/lib/rfsa/adsp;/dsp;/vendor/lib";
...
Java_com_example_persondetection_MainActivity_setSurface(
        JNIEnv* env, jobject clazz,
        jobject surface) {

    app.SetNativeWindow(ANativeWindow_fromSurface(env, surface));
    app.SetUpCamera();
    std::thread loopThread(&Person_Detect::CameraLoop, &app);
    loopThread.detach();
}
